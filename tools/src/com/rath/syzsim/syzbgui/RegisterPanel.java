  ...
  public RegisterPanel(final Dimension size, final SyzInternals si) {
    ...
    for (int i = 0; i < regLabels.length / 2; i++) {
      ...
      for (int j = 0; j <= 1; j++) {
        ...
        regLabels[reg].setBounds(PADDING_REG_NUM_W + (j * PADDING_REG_ODDS),
            PADDING_REGS_N + REG_HEIGHT * i, LABEL_WIDTH, FIELD_HEIGHT);
        regLabels[reg].setFont(MemoryPanel.FONT_MONOSPACED);
        add(regLabels[reg]);
        
        // Text fields
        regFields[reg] = new JTextField(5);
        regFields[reg].setBounds(PADDING_REG_FIELD_W + (j * PADDING_REG_ODDS),
            PADDING_REGS_N + REG_HEIGHT * i, FIELD_WIDTH, FIELD_HEIGHT);
        add(regFields[reg]);
        
        // Value labels
        final short regByte = this.internals.getReg(reg);
        regVals[reg] = new JLabel("(" + regByte + ")");
        regVals[reg].setBounds(PADDING_REG_VAL_W + (j * PADDING_REG_ODDS),
            PADDING_REGS_N + REG_HEIGHT * i, VALUE_WIDTH, FIELD_HEIGHT);
        regVals[reg].setFont(MemoryPanel.FONT_MONOSPACED);
        add(regVals[reg]);
        
        // Field key listeners
        regFields[reg]
            .addKeyListener(new RegKeyListener(this.internals, regVals[reg], regFields[reg], reg));
      }
    }
  }
